...
const StartStore: React.FC<StartStoreProps> = ({ onClose }) => {
  ...
  const handleBuyClick = async () => {
    try {
      ...
      await fetch("https://lionsbackend.lionsgangbot.com/update_gamer", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="start-store-overlay">
      <div className="start-store-container bg-black/40 bg-opacity-60 backdrop-filter backdrop-blur-lg">
        <div className="start-store-header">
          <IoArrowBackSharp
            className="back-arrow text-white"
            onClick={onClose}
          />
          <h2 className="store-title text-white/80">
        ...
        <div className="reward-card bg-white border border-[#ffffffd3] ">
          ...
          <p className="reward-text text-white/80">
